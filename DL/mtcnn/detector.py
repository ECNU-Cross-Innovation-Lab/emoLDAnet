...
class FaceDetector:
    ...
    def new_detect(
        ...
        for image in images:
            ...
            print('\r' + str(tmp), end='', flush=True)
            ...
            if bboxes is None or not len(bboxes):
                continue
            boxes = [b for b in convert_to_square(bboxes) if len(b) == 5]
            if not len(boxes):
                continue
            x1, y1, x2, y2, p = sorted(boxes, key=lambda x: -x[4])[0]
            if p < 0.99:
                continue

            ret_images.append(np.array(image.crop((x1, y1, x2, y2))))
        return ret_images
